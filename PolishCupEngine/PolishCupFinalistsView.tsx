...
export const PolishCupFinalistsView: React.FC = () => {
  const { clubs, fixtures, userTeamId, navigateTo } = useGame();

  const getClub = (id: string) => clubs.find(c => c.id === id);

  // Wyznacz finalistów: drużyny z isInPolishCup lub ze zwycięskich 1/2 finału
  const finalists = useMemo(() => {
    const sfReturn = fixtures.filter(
      f => f.leagueId === CompetitionType.POLISH_CUP &&
           f.id.includes('1/2') &&
           f.status === MatchStatus.FINISHED,
    );

    if (sfReturn.length >= 2) {
      return sfReturn.map(f => {
    ...
    // fallback: isInPolishCup
    return clubs.filter(c => c.isInPolishCup).map(c => c.id);
  }, [fixtures, clubs]);
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden relative">
      ...
      <div className="fixed inset-0 z-0">
        <img src={PucharPolskiBg} alt="" className="w-full h-full object-cover opacity-30" />
      ...
      <div className="relative z-10 flex flex-col h-full p-6 gap-4">
        ...
        <div className={GLASS_CARD + " p-6 flex items-center justify-between shrink-0"}>
          ...
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 rounded-2xl bg-rose-600/10 border border-rose-500/30 flex items-center justify-center text-3xl">
              🏆
            </div>
            <div>
              <p className="text-rose-500 text-[9px] font-black uppercase tracking-[0.5em]">Puchar Polski</p>
              <h1 className="text-3xl font-black italic uppercase tracking-tighter text-white leading-none">
                Ogłoszenie Finalistów
              </h1>
              <p className="text-slate-400 text-xs mt-1">
                Para finałowa wyłoniona · Mecz jednorazowy · 2 maja
              </p>
            </div>
          </div>
          <button
            onClick={() => navigateTo(ViewState.DASHBOARD)}
            className="px-10 py-4 bg-rose-600 hover:bg-rose-500 text-white font-black italic uppercase tracking-widest rounded-2xl shadow-2xl transition-all hover:scale-105 active:scale-95 text-sm"
          >
            DALEJ →
          </button>
        </div>
          ...
          <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
            ...
            {userIsFinalist && (
              <div className="text-center">
                <span className="inline-block px-5 py-2 rounded-full bg-rose-600/10 border border-rose-500/30 text-rose-400 text-[10px] font-black uppercase tracking-[0.4em]">
              ...
              <div className="flex items-center px-6 py-6 bg-white/[0.04] border border-white/[0.08] rounded-3xl gap-4">
                ...
                <div className="flex items-center gap-3 flex-1 justify-end">
                  {finalists[0] === userTeamId && (
                    <span className="text-rose-400 text-[9px] font-black uppercase tracking-[0.3em] shrink-0">TY</span>
                  )}
            ...
            <p className="text-[8px] font-black uppercase tracking-[0.3em] text-slate-600 text-center mt-2">
              Miejsce: Neutralny stadion · Format: Mecz jednorazowy · Ew. rzuty karne
            </p>
          </div>
        </div>
