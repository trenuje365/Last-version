...
export const PreMatchCupStudioView: React.FC = () => {
  ...
  const bettingData = useMemo(() => {
    ...
    // Nowa waga: Średni OVR (60%) + Reputacja (30%) + Bonus Domowy (tylko 10%)
    // Bonus domowy obniżony z +5 na +2 (bardziej realistyczny dla pucharów)
    let hPower = hAvg + (hRep * 2.5) + (data.fixture.id.includes("FINAŁ") ? 0 : 2); 
    let aPower = aAvg + (aRep * 2.5);
