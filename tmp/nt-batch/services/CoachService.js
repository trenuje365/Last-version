...
const randomTactic = (list) => list[Math.floor(Math.random() * list.length)];
exports.CoachService = {
    generateInitialCoaches: (clubs) => {
        const coaches = {};
        const coachList = [];
        for (let i = 0; i < 1500; i++) {
            coachList.push(exports.CoachService.createRandomCoach(i < 180));
        }
        ...
        updatedClubs.forEach(club => {
            ...
            if (club.leagueId === 'L_CL' || club.leagueId === 'L_EL' || club.leagueId === 'L_CONF') {
                ...
                if (club.reputation >= 18) {
                    minExp = 80;
                    maxExp = 99;
                } // Tier 1 top (Real, Bayern, PSG)
                else if (club.reputation >= 15) {
                    minExp = 70;
                    maxExp = 88;
                } // Tier 1 (Porto, Benfica)
                else if (club.reputation >= 12) {
                    minExp = 48;
                    maxExp = 75;
                } // Tier 2 (Club Brugge, Dinamo)
                else {
                    minExp = 10;
                    maxExp = 60;
                } // Tier 3/4 (Sheriff, Žalgiris)
            }
            else {
                // Polskie kluby — stara logika
                if (club.reputation >= 7)
                    maxExp = 72;
            ...
            const polishCandidates = isPolishClub ? coachList.filter(c => c.attributes.experience >= minExp &&
                c.attributes.experience <= maxExp &&
                c.currentClubId === null &&
                c.nationality === types_1.Region.POLAND) : [];
            const candidates = polishCandidates.length > 0
                ? polishCandidates
                : coachList.filter(c => c.attributes.experience >= minExp &&
                    c.attributes.experience <= maxExp &&
                    c.currentClubId === null &&
                    (!excludePolish || c.nationality !== types_1.Region.POLAND));
            ...
            while (finalCandidates.length === 0 && searchMinExp > 0) {
                ...
                finalCandidates = coachList.filter(c => c.attributes.experience >= searchMinExp &&
                    c.attributes.experience <= maxExp &&
                    c.currentClubId === null &&
                    (!excludePolish || c.nationality !== types_1.Region.POLAND));
            }
    ...
    createRandomCoach: (isPolish) => {
        const region = isPolish ? types_1.Region.POLAND : NameGeneratorService_1.NameGeneratorService.getRandomForeignRegion();
        const namePair = NameGeneratorService_1.NameGeneratorService.getRandomName(region);
        return {
    ...
    generateNationalTeamCoaches: () => {
        ...
        tiers.forEach(({ minExp, maxExp, count }) => {
            for (let i = 0; i < count; i++) {
                const region = NameGeneratorService_1.NameGeneratorService.getRandomForeignRegion();
                const namePair = NameGeneratorService_1.NameGeneratorService.getRandomName(region);
    ...
    evaluatePerformance: (club, coach, rank) => {
        ...
        if (distance >= 8) {
            ...
            if (chance < 0.4)
                return { fire: true, reason: "Brak wyników sportowych i niezadowolenie kibiców." };
        }
        // Katastrofa: strefa spadkowa przy wysokiej reputacji
        if (rank >= 16 && club.reputation >= 7) {
            if (Math.random() < 0.7)
                return { fire: true, reason: "Kompromitująca pozycja w tabeli względem potencjału." };
        }
