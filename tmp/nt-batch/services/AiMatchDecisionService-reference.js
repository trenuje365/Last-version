import { PlayerPosition, InjurySeverity } from '../types';
...
export const AiMatchDecisionService = {
     ...
     */
    makeDecisions: (state, ctx, side, isPriority = false, isHalftime = false) => {
        ...
        if (Math.random() > reactionChance && !isPriority)
            return { logs: [] };
        ...
        if (gkInSlot === null) {
            const bestGkOnBench = newLineup.bench
                .map(id => myPlayers.find(p => p.id === id)).filter((p) => !!p)
            ...
            if (bestGkOnBench && currentSubsCount < 5) {
                ...
                for (let i = newLineup.startingXI.length - 1; i >= 1; i--) {
                    if (newLineup.startingXI[i] !== null) {
                        fieldPlayerIdx = i;
                        break;
                    }
                }
                if (fieldPlayerIdx !== -1) {
                    ...
                    logs.push(`${state.minute}' Bramkarz rezerwowy ${bestGkOnBench.lastName} zastępuje gracza z pola!`);
                }
            }
            // OPCJA B: Brak zmian LUB brak GK na ławce -> Gracz z pola musi wejść do bramki
                ...
                if (fieldCandidates.length > 0) {
                    ...
                    logs.push(`${state.minute}' Niecodzienna sytuacja na boisku. ${bestCandidate.p.lastName} musi stanąć między słupkami!`);
                }
            }
        }
        ...
        if (scoreDiff > 0 && mySentOffCount > 0) {
            ...
            if (newLineup.tacticId !== '5-4-1' && newLineup.tacticId !== '6-3-1') {
                newTacticId = '5-4-1';
                logs.push(`${state.minute}'Zmian taktyki na 5-4-1.`);
            }
            ...
            if (emptyDefIdx !== undefined && !subRecord) {
                ...
                if (newSubsCount < 5) {
                    ...
                    if (bestDefOnBench) {
                        ...
                        if (sacrificeIdx !== -1) {
                            ...
                            logs.push(`${state.minute}'Zmiana wymuszona sytuacją. ${bestDefOnBench.lastName} wchodzi do obrony.`);
                        }
                    }
                }
                // Opcja 2: Brak zmian -> Przesuwamy najlepszego defensywnego pomocnika do obrony
                    ...
                    if (bestInternalCover) {
                        ...
                        logs.push(`${state.minute}' Brak zmian. ${bestInternalCover.p.lastName} musi zagrać w obronie.`);
                    }
                }
            }
        }
            ...
            if (emptySlotIdx !== -1) {
                ...
                if (currentOnPitchCount < maxAllowedOnPitch) {
                    ...
                    if (bestSub) {
                        ...
                        logs.push(`${state.minute}' Zmiana, ${bestSub.lastName} wchodzi w miejsce zniesionego gracza.`);
                    }
                }
            }
        }
            ...
            if (playerOutId) {
                ...
                if (requiredRole === PlayerPosition.GK) {
                    ...
                    bestSub = benchPool
                        ...
                        if (scoreDiff < 0 && b.position === PlayerPosition.FWD)
                            scoreB += 25;
