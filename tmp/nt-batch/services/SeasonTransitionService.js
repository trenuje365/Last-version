import { PlayerPosition, HealthStatus, Region } from '../types';
...
export const SeasonTransitionService = {
     ...
     */
    processSquadTransition: (playersMap, clubs, seasonEndDate, userTeamId) => {
        const updatedMap = { ...playersMap };
        const logs = [];
        const releasedPlayers = []; // ← NOWA LINIA
        for (const clubId in updatedMap) {
            ...
            if (club.leagueId === 'L_PL_1')
                leagueTier = 1;
            else if (club.leagueId === 'L_PL_2')
                leagueTier = 2;
            else if (club.leagueId === 'L_PL_3')
                leagueTier = 3;
            ...
            currentSquad.forEach(player => {
                ...
                if (player.age >= 35 && Math.random() < 0.5) {
                    ...
                    const newgen = SeasonTransitionService.generateNewgen(clubId, player.position, leagueTier, club.reputation, club.budget, nextSquad.length);
     ...
     */
    generateNewgen: (clubId, position, tier, reputation, clubBudget, index) => {
    ...
    /**
       ...
       */
    generateEmergencyGK: (clubId, tier, reputation) => {
