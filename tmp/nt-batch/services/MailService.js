...
export const MailService = {
     ...
     */
    generateWelcomeMail: (userClub, squad, gameDate) => {
    ...
    /**
       ...
       */
    generateFanWelcomeMail: (userClub, squad, gameDate) => {
    ...
    generateBoardDecisionMail: (player, club, decision) => {
        ...
        if (decision.status === 'VETO' || decision.status === 'SOFT_BLOCK')
            templateId = 'board_bie_veto';
     ...
     */
    generateDailyMails: (currentDate, userClub, allPlayers, allClubs, rank, boardConfidence, recentFixture, existingMails = []) => {
        const newMails = [];
        ...
        if (rng < 0.2) {
            ...
            if (victim && victimClub) {
                const alreadySent = existingMails.some(m => m.subject.includes(victim.lastName));
                if (!alreadySent) {
        ...
        if (severeInjury && rng < 0.15) {
            const alreadySentSevere = existingMails.some(m => m.subject.includes(severeInjury.lastName));
    ...
    generateIncomingOfferMail(player, buyerClubName, buyerLeagueName, fee, timing, sellerClubName, boardPressure, currentDate, offerId) {
    ...
    generateIncomingOfferReminderMail(player, buyerClubName, fee, sellerClubName, currentDate, offerId) {
    ...
    generateIncomingOfferExpiredMail(player, buyerClubName, sellerClubName, currentDate) {
    ...
    generateAIAcceptedCounterMail(player, buyerClubName, fee, sellerClubName, currentDate, offerId) {
    ...
    generateAICounteredMail(player, buyerClubName, aiCounterFee, round, sellerClubName, currentDate, offerId) {
    ...
    generateAIRejectedCounterMail(player, buyerClubName, sellerClubName, currentDate) {
    ...
    generatePlayerAcceptedConfirmMail(player, buyerClubName, fee, timing, sellerClubName, currentDate, offerId) {
    ...
    generatePlayerRefusedMail(player, buyerClubName, sellerClubName, currentDate) {
