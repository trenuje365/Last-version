...
export const MomentumService = {
    ...
    calculateNaturalTarget: (ctx, state) => {
        ...
        const homeHasGK = ctx.homePlayers.find((p) => p.id === state.homeLineup.startingXI[0])?.position === 'GK';
        const awayHasGK = ctx.awayPlayers.find((p) => p.id === state.awayLineup.startingXI[0])?.position === 'GK';
     ...
     */
    computeMomentum: (ctx, state, lastEventType, lastEventSide) => {
