import { SlotType, CompetitionType } from '../types';
...
const getDayOfWeek = (date) => date.getDay(); // 0 = Sun, 1 = Mon, ... 6 = Sat
/**
 * Returns the Weekend Range (Sat-Sun) containing the given date.
 */
const getWeekendContaining = (date) => {
...
export const SeasonTemplateGenerator = {
    generate: (seasonStartYear) => {
        ...
        const addSlot = (start, end, type, comp, label, priority, meta) => {
            slots.push({
        ...
        addSlot(new Date(seasonStartYear, 6, 9), // 9 Lipca
        new Date(seasonStartYear, 6, 9), SlotType.MIDWEEK, CompetitionType.BREAK, "Przygotowanie do sezonu", 100);
        ...
        for (let d = 14; d <= 18; d++) {
            ...
            addSlot(prepDate, prepDate, SlotType.MIDWEEK, CompetitionType.TRANSFER_WINDOW, "Letnie przygotowania i transfery", 75);
        }
        ...
        const scheduleCupRound = (baseDate, offsetWeeks, label) => {
        ...
        const scheduleEuroRound = (baseDate, offsetWeeks, label) => {
            let targetStart = addDays(baseDate, offsetWeeks * 7);
