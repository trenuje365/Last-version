 ...
 */
import { CompetitionType, EventKind, ViewState, MatchStatus, } from '../types';
...
export const CalendarEngine = {
     ...
     */
    getEventsForDate(date, seasonTemplate, allFixtures, userTeamId, clubs) {
        if (!seasonTemplate || !userTeamId)
            return [];
        ...
        const order = { player: 0, background: 1, info: 2 };
     ...
     */
    getNextPlayerEvent(currentDate, userTeamId, userTier, schedules, seasonTemplate, allFixtures, clubs) {
        if (!seasonTemplate || !userTeamId)
            return null;
        ...
        for (const slot of upcoming) {
            ...
            if (slot.competition === CompetitionType.LEAGUE) {
                ...
                const matchday = schedule.matchdays.find(md => {
                    ...
                    return (mdStart >= now &&
                        md.fixtures.some(f => (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                            f.status === MatchStatus.SCHEDULED));
                });
                if (!matchday)
                    continue;
                const fixture = matchday.fixtures.find(f => (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                return {
            ...
            if (!ev)
                continue;
            if (ev.participation === 'info')
                continue; // pomijamy czysto informacyjne
            if (ev.participation === 'background')
                continue; // pomijamy tło — te dni auto-przeskakują podczas jumpToDate
            return {
     ...
     */
    _resolveSlot(slot, date, allFixtures, userTeamId, clubs) {
        const dateStr = date.toDateString();
        const userClub = clubs.find(c => c.id === userTeamId);
        switch (slot.competition) {
            // ── LIGA ───────────────────────────────────────────────────────────────
            case CompetitionType.LEAGUE: {
                const fixture = allFixtures.find(f => f.date.toDateString() === dateStr &&
            ...
            case CompetitionType.SUPER_CUP: {
                ...
                const fixture = allFixtures.find(f => f.id.startsWith(`SUPER_CUP_${year}`) && f.status === MatchStatus.SCHEDULED);
                if (!fixture)
                    return null;
            ...
            case CompetitionType.POLISH_CUP: {
                ...
                const hasAnyCupToday = allFixtures.some(f => f.date.toDateString() === dateStr &&
            ...
            case CompetitionType.EL_R1Q_RETURN: {
                const leagueIdELR1Q = slot.competition;
                const fixtureELR1Q = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdELR1Q &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyELR1Q = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.EL_R1Q || f.leagueId === CompetitionType.EL_R1Q_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureELR1Q && !hasAnyELR1Q)
                    return null;
                return {
                    slot,
            ...
            case CompetitionType.EL_R2Q_RETURN: {
                const leagueIdELR2Q = slot.competition;
                const fixtureELR2Q = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdELR2Q &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyELR2Q = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.EL_R2Q || f.leagueId === CompetitionType.EL_R2Q_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureELR2Q && !hasAnyELR2Q)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.EL_GROUP_STAGE: {
                const fixtureELGS = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === CompetitionType.EL_GROUP_STAGE &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyELGS = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    f.leagueId === CompetitionType.EL_GROUP_STAGE &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureELGS && !hasAnyELGS)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.EL_R16_RETURN: {
                const leagueIdELR16 = slot.competition;
                const fixtureELR16 = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdELR16 &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyELR16 = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.EL_R16 || f.leagueId === CompetitionType.EL_R16_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureELR16 && !hasAnyELR16)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.EL_SF_RETURN: {
                const leagueIdEL = slot.competition;
                const fixtureEL = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdEL &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyEL = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdEL &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureEL && !hasAnyEL)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.EL_FINAL: {
                const fixtureELFinal = allFixtures.find(f => f.date.toDateString() === dateStr &&
            ...
            case CompetitionType.CL_R2Q_RETURN: {
                const leagueId = slot.competition;
                const fixture = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueId &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
            ...
            case CompetitionType.CL_GROUP_STAGE: {
                ...
                const hasAnyScheduled = allFixtures.some(f => f.date.toDateString() === dateStr &&
            ...
            case CompetitionType.CONF_R1Q_RETURN: {
                const leagueIdCONFR1Q = slot.competition;
                const fixtureCONFR1Q = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdCONFR1Q &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFR1Q = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.CONF_R1Q || f.leagueId === CompetitionType.CONF_R1Q_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFR1Q && !hasAnyCONFR1Q)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_R2Q_RETURN: {
                const leagueIdCONFR2Q = slot.competition;
                const fixtureCONFR2Q = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdCONFR2Q &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFR2Q = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.CONF_R2Q || f.leagueId === CompetitionType.CONF_R2Q_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFR2Q && !hasAnyCONFR2Q)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_GROUP_STAGE: {
                const fixtureCONFGS = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === CompetitionType.CONF_GROUP_STAGE &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFGS = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    f.leagueId === CompetitionType.CONF_GROUP_STAGE &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFGS && !hasAnyCONFGS)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_R16_RETURN: {
                const leagueIdCONFR16 = slot.competition;
                const fixtureCONFR16 = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdCONFR16 &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFR16 = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.CONF_R16 || f.leagueId === CompetitionType.CONF_R16_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFR16 && !hasAnyCONFR16)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_QF_RETURN: {
                const leagueIdCONFQF = slot.competition;
                const fixtureCONFQF = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdCONFQF &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFQF = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.CONF_QF || f.leagueId === CompetitionType.CONF_QF_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFQF && !hasAnyCONFQF)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_SF_RETURN: {
                const leagueIdCONFSF = slot.competition;
                const fixtureCONFSF = allFixtures.find(f => f.date.toDateString() === dateStr &&
                    f.leagueId === leagueIdCONFSF &&
                    (f.homeTeamId === userTeamId || f.awayTeamId === userTeamId) &&
                    f.status === MatchStatus.SCHEDULED);
                const hasAnyCONFSF = allFixtures.some(f => f.date.toDateString() === dateStr &&
                    (f.leagueId === CompetitionType.CONF_SF || f.leagueId === CompetitionType.CONF_SF_RETURN) &&
                    f.status === MatchStatus.SCHEDULED);
                if (!fixtureCONFSF && !hasAnyCONFSF)
                    return null;
                return {
                    slot,
                    kind: EventKind.MATCH_EURO,
            ...
            case CompetitionType.CONF_FINAL: {
                const fixtureCONFFinal = allFixtures.find(f => f.date.toDateString() === dateStr &&
     ...
     */
    getPrimaryEventForDate(date, seasonTemplate, allFixtures, userTeamId, clubs) {
