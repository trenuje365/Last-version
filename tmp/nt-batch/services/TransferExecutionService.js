...
const buildFinanceLog = (amount, description, date, previousBalance) => ({
...
export const TransferExecutionService = {
    finalizeTransfer: (offer, clubs, playersMap, currentDate) => {
        ...
        const updatedClubs = clubs.map(club => {
            if (club.id === buyerClub.id) {
                const buyerFeeLog = buildFinanceLog(-offer.fee, `Kwota transferu za ${player.firstName} ${player.lastName}`, currentDate, club.budget);
                const buyerBonusLog = buildFinanceLog(-offer.bonus, `Bonus za podpis dla ${player.firstName} ${player.lastName}`, currentDate, club.budget - offer.fee);
                return {
                    ...club,
            ...
            if (club.id === sellerClub.id) {
                const sellerIncomeLog = buildFinanceLog(offer.fee, `Sprzedaż zawodnika ${player.firstName} ${player.lastName}`, currentDate, club.budget);
                return {
                    ...club,
