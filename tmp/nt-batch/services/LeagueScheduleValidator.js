    ...
    validate: (schedule) => {
        ...
        schedule.matchdays.forEach(md => {
            ...
            md.fixtures.forEach(fix => {
                ...
                if (teamsInRound.has(fix.homeTeamId))
                    errors.push(`Round ${md.roundNumber}: Team ${fix.homeTeamId} plays twice.`);
                if (teamsInRound.has(fix.awayTeamId))
                    errors.push(`Round ${md.roundNumber}: Team ${fix.awayTeamId} plays twice.`);
