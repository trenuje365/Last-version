...
export const MomentumService = {
    ...
    calculateNaturalTarget: (ctx, state) => {
        ...
        if (homeTactic.attackBias > 65 && techGap < -8)
            target += techGap < -15 ? -12 : -6;
        if (homeTactic.defenseBias > 65 && techGap < -8)
            target += techGap < -15 ? 8 : 4;
        if (awayTactic.attackBias > 65 && techGap > 8)
            target += techGap > 15 ? 12 : 6;
        if (awayTactic.defenseBias > 65 && techGap > 8)
            target += techGap > 15 ? -8 : -4;
        // --- FORMA DRUŻYNY ---
        // Liczymy serię wygranych/przegranych od ostatniego meczu (koniec tablicy = najnowszy)
        const getFormBonus = (form) => {
     ...
     */
    computeMomentum: (ctx, state, lastEventType, lastEventSide, homeFatigueMap, awayFatigueMap) => {
        ...
        const _homeAvgMentality = state.homeLineup.startingXI.filter((id) => id !== null).reduce((acc, id) => { const p = ctx.homePlayers.find(x => x.id === id); return acc + (p?.attributes.mentality ?? 50); }, 0) / Math.max(1, state.homeLineup.startingXI.filter(id => id !== null).length);
        const _awayAvgMentality = state.awayLineup.startingXI.filter((id) => id !== null).reduce((acc, id) => { const p = ctx.awayPlayers.find(x => x.id === id); return acc + (p?.attributes.mentality ?? 50); }, 0) / Math.max(1, state.awayLineup.startingXI.filter(id => id !== null).length);
        ...
        const fatiguePenalty = (avg) => {
            if (avg < 35)
                return 8; // czerwony pasek → duży bonus dla rywala
            if (avg < 50)
                return 5; // pomarańczowy dolny → wyraźny bonus
            if (avg < 70)
                return 2; // pomarańczowy górny → lekki bonus
            return 0;
        };
