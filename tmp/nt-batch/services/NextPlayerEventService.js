 ...
 */
import { MatchStatus, EventKind, CompetitionType } from '../types';
export const NextPlayerEventService = {
    getNextEvent: (currentDate, userTeamId, userTier, schedules, seasonTemplate, fixtures = []) => {
            ...
            seasonTemplate.slots.forEach(slot => {
                ...
                switch (slot.competition) {
                    ...
                    case CompetitionType.CL_GROUP_STAGE: {
                        const hasScheduledCLGs = fixtures.some(f => f.date.toDateString() === slot.start.toDateString() &&
                    ...
                    case CompetitionType.POLISH_CUP:
                        if (!slot.label.includes("LOSOWANIE") && !slot.label.includes("Ogłoszenie")) {
                            const hasUserCupFixture = fixtures.some(f => f.date.toDateString() === slot.start.toDateString() &&
