...
export const GoalAttributionService = {
     ...
     */
    checkShotSuccess: (attacker, goalkeeper, defenders, isHeader, rng, isPenalty = false, scorerLiveFatigue = 100, gkLiveFatigue = 100, scorerFitMod = 1.0, gkFitMod = 1.0, defFatigueMap = {}) => {
        // Failsafe dla brakujących graczy (np. po czerwonej kartce przed zmianą AI)
        if (!attacker)
            return false;
        if (!goalkeeper)
            return rng() < 0.98; // Brak bramkarza = prawie pewny gol
