...
export const TransferBuyerLogicService = {
    validateClubBid: (player, buyerClub, buyerSquad, offer, currentDate) => {
        ...
        const fairFee = Math.max(player.marketValue || 0, Math.max(50_000, FinanceService.getFairMarketSalary(player.overallRating) * 6));
    ...
    validateContractTerms: (player, buyerClub, buyerSquad, contract) => {
        ...
        const boardDecision = FinanceService.evaluateFASigningBoardDecision(player, contract.salary, contract.bonus, buyerSquad, buyerClub);
