...
const checkTacticFeasibility = (players, tacticId) => {
    const tactic = tactics_db_1.TacticRepository.getById(tacticId);
    const required = {};
    ...
    const available = {};
    players.forEach(p => {
        if (p.position !== types_1.PlayerPosition.GK) {
...
exports.LineupService = {
     ...
     */
    autoPickLineup: (clubId, players, tacticId = '4-4-2', coach = null) => {
        ...
        const availablePlayers = players.filter(p => (p.suspensionMatches || 0) === 0 &&
            p.condition >= 60 &&
            (p.health.status === types_1.HealthStatus.HEALTHY || (p.health.injury?.severity !== types_1.InjurySeverity.SEVERE && (p.health.injury?.daysRemaining ?? 0) <= 2)));
        const COND_XI = 90;
        const COND_BENCH = 85;
        ...
        const gkXI = poolXI.find(p => p.position === types_1.PlayerPosition.GK);
        const gkBench = poolBench.find(p => p.position === types_1.PlayerPosition.GK);
        const gkRest = poolRest.find(p => p.position === types_1.PlayerPosition.GK);
        ...
        for (let i = 1; i < 11; i++) {
            ...
            const candidate = poolXI.find(p => !usedPlayerIds.has(p.id) && p.position === requiredRole) ??
        ...
        const addToBench = (p) => { bench.push(p.id); usedPlayerIds.add(p.id); };
        const findBench = (pos) => benchEligible.find(p => !usedPlayerIds.has(p.id) && (pos === null || p.position === pos)) ??
            poolRest.find(p => !usedPlayerIds.has(p.id) && (pos === null || p.position === pos));
        // Slot 1: Bramkarz (obowiązkowy)
        const bGK = findBench(types_1.PlayerPosition.GK);
        if (bGK)
            addToBench(bGK);
        // Sloty 2-4: Minimum 1 obrońca, 1 pomocnik, 1 napastnik
        const mandatoryPositions = [types_1.PlayerPosition.DEF, types_1.PlayerPosition.MID, types_1.PlayerPosition.FWD];
        ...
        for (const p of benchEligible) {
            if (bench.length >= 9)
                break;
            if (!usedPlayerIds.has(p.id) && p.position !== types_1.PlayerPosition.GK)
                addToBench(p);
        }
    ...
    calculateFitScore: (player, role) => {
        const attr = player.attributes;
        const isGkPlayer = player.position === types_1.PlayerPosition.GK;
        const isGkRole = role === types_1.PlayerPosition.GK;
        if ((isGkPlayer && !isGkRole) || (!isGkPlayer && isGkRole)) {
            return -2000 + player.overallRating;
        }
        switch (role) {
            case types_1.PlayerPosition.GK:
                return attr.goalkeeping * 2 + attr.positioning;
            case types_1.PlayerPosition.DEF:
                return attr.defending * 1.5 + attr.strength + attr.positioning;
            case types_1.PlayerPosition.MID:
                return attr.passing * 1.2 + attr.vision + attr.technique;
            case types_1.PlayerPosition.FWD:
    ...
    /**
       ...
       */
    repairLineup: (lineup, players) => {
        const AI_FRESH_THRESHOLD = 87;
        const tactic = tactics_db_1.TacticRepository.getById(lineup.tacticId);
        const canPlay = (p) => (p.suspensionMatches || 0) === 0 && p.condition >= 60 && (p.health.status !== types_1.HealthStatus.INJURED || p.health.injury?.severity !== types_1.InjurySeverity.SEVERE);
        ...
        const freshGk = freshPool.find(p => p.position === types_1.PlayerPosition.GK);
        const bestGk = freshGk || tiredPool.find(p => p.position === types_1.PlayerPosition.GK);
        if (bestGk) {
            newXI[0] = bestGk.id;
        ...
        for (let i = 1; i < 11; i++) {
            if (!newXI[i]) {
                const match = freshPool.find(p => p.position !== types_1.PlayerPosition.GK && !usedIds.has(p.id));
                if (match) {
                    newXI[i] = match.id;
                    usedIds.add(match.id);
                }
        ...
        const newBench = [];
        const benchTarget = [types_1.PlayerPosition.GK, types_1.PlayerPosition.DEF, types_1.PlayerPosition.DEF, types_1.PlayerPosition.MID, types_1.PlayerPosition.MID, types_1.PlayerPosition.MID, types_1.PlayerPosition.FWD, types_1.PlayerPosition.FWD, types_1.PlayerPosition.MID];
    ...
    evictSuspendedPlayers: (lineup, players) => {
        ...
        const isRestricted = (id) => {
            ...
            const isSevereInjured = p.health.status === types_1.HealthStatus.INJURED && p.health.injury?.severity === types_1.InjurySeverity.SEVERE;
            return isSuspended || isSevereInjured;
        };
        newLineup.startingXI = newLineup.startingXI.map(id => {
            if (id && isRestricted(id)) {
                if (!newLineup.reserves.includes(id))
                    newLineup.reserves.push(id);
                return null;
            }
            return id;
        });
    ...
    validateLineup: (lineup, allClubPlayers) => {
        const missingCount = lineup.startingXI.filter(id => id === null).length;
        if (missingCount > 0)
            return { valid: false, error: `Skład niekompletny! Brakuje ${missingCount} zawodników.` };
        if (lineup.bench.length > 9)
            return { valid: false, error: "Zbyt wielu zawodników na ławce" };
        const startPlayers = allClubPlayers.filter(p => lineup.startingXI.includes(p.id));
        const hasGK = startPlayers.some(p => p.position === types_1.PlayerPosition.GK);
        if (!hasGK)
            return { valid: false, error: "Brak bramkarza w podstawowej jedenastce!" };
        if (startPlayers.some(p => (p.suspensionMatches || 0) > 0))
            return { valid: false, error: "W wyjściowym składzie znajduje się zawieszony zawodnik!" };
        if (startPlayers.some(p => p.health.status === types_1.HealthStatus.INJURED && p.health.injury?.severity === types_1.InjurySeverity.SEVERE))
            return { valid: false, error: "W wyjściowym składzie znajduje się poważnie kontuzjowany zawodnik!" };
        return { valid: true };
    },
    assignToSlot: (lineup, playerId, slotIdx) => {
        ...
        if (oldOccupant)
            newLineup.reserves.push(oldOccupant);
    ...
    swapPlayers: (lineup, sourceId, targetId, sourceIdx, targetIdx) => {
        ...
        for (let i = 0; i < 11; i++) {
            if (sourceIdx === i || (sourceId !== null && nextLineup.startingXI[i] === sourceId))
                nextLineup.startingXI[i] = null;
            else if (targetIdx === i || (targetId !== null && nextLineup.startingXI[i] === targetId))
                nextLineup.startingXI[i] = null;
        }
        if (sourceId) {
        ...
        if (targetIdx !== undefined && targetIdx < 11) {
            ...
            if (sourceId)
                nextLineup.bench.push(sourceId);
        }
        else {
            if (sourceId)
                nextLineup.reserves.push(sourceId);
        }
        if (sourceIdx !== undefined && sourceIdx < 11) {
            ...
            if (targetId)
                nextLineup.bench.push(targetId);
        }
        else {
            if (targetId)
                nextLineup.reserves.push(targetId);
        }
        ...
        while (nextLineup.bench.length > 9) {
            ...
            if (extra)
                nextLineup.reserves.push(extra);
        }
        return nextLineup;
    }
};
