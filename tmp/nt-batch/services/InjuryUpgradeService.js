import { MatchEventType, InjurySeverity } from '../types';
export const InjuryUpgradeService = {
        ...
        const processTeam = (side) => {
            ...
            lineup.forEach(playerId => {
                ...
                if (injuries[playerId] === InjurySeverity.LIGHT && riskModes[playerId]) {
                    ...
                    if (minsSinceInjury < 3)
                        p *= 0.6;
                    else if (minsSinceInjury > 15)
                        p *= 1.2;
                    // Clamp
     ...
     */
    calculateUpgradeProbability: (player, fatigue, baseProb, currentMin, injuryMin) => {
        let p = baseProb;
        const staminaNorm = (player.attributes.stamina || 50) / 100;
        if (fatigue < 25)
            p *= 1.9;
