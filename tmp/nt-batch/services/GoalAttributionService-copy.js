...
export const GoalAttributionService = {
     ...
     */
    checkShotSuccess: (attacker, goalkeeper, defenders, isHeader, rng, isPenalty = false) => {
        // 1. Dedykowana logika rzutu karnego (ok. 95% szansy)
        // TUTAJ WSTAW TEN KOD:
        // OBLICZANIE KAR ZA PRZEMĘCZENIE (Stage 1 Pro)
        const calculateFatigueImpact = (p) => {
            const debt = p.fatigueDebt || 0;
            if (debt > 40)
                return 0.70; // -30% do statystyk przy ogromnym długu
            if (debt > 20)
                return 0.85; // -15% do statystyk
            if (debt > 10)
                return 0.95; // -5% do statystyk
            return 1.0;
        };
    ...
    pickScorer: (players, lineupIds, isCorner, rng) => {
        ...
        const weights = candidates.map(p => {
            ...
            if (isCorner) {
                ...
                if (p.position === PlayerPosition.DEF)
                    w *= 2.2;
            }
            else {
