...
export const MatchEngineService = {
    calculateFatigueStep: (state, ctx, weather) => {
        ...
        const update = (players, fatigueMap, sideLineup, pressureFactor, pressingMod, redFatigueMod) => {
            players.forEach(p => {
                if (!sideLineup.includes(p.id))
                    return;
                ...
                drain *= pressingMod;
                drain *= redFatigueMod;
                if (weather && weather.precipitationChance > 0)
                    drain *= 1.08;
                if (weather && weather.tempC > 30)
                    drain *= 1.10;
