import { PlayerPosition, InjurySeverity, HealthStatus } from '../types';
import { TacticRepository } from '../resources/tactics_db';
export const LineupService = {
    /**
     * Deterministyczny wybór składu.
     */
    autoPickLineup: (clubId, players, tacticId = '4-4-2') => {
        ...
        for (let i = 1; i < 11; i++) {
            ...
            const candidate = sortedPlayers.find(p => !usedPlayerIds.has(p.id) && p.position === requiredRole);
            if (candidate) {
        ...
        const benchGK = sortedPlayers.find(p => p.position === PlayerPosition.GK && !usedPlayerIds.has(p.id));
        if (benchGK) {
    ...
    repairLineup: (lineup, players) => {
        ...
        const canPlay = (p) => (p.suspensionMatches || 0) === 0 && (p.health.status !== HealthStatus.INJURED || p.health.injury?.severity !== InjurySeverity.SEVERE);
    ...
    validateLineup: (lineup, allClubPlayers) => {
        ...
        if (startPlayers.some(p => p.health.status === HealthStatus.INJURED && p.health.injury?.severity === InjurySeverity.SEVERE))
