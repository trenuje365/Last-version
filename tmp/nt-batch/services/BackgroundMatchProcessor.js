import { MatchStatus, CompetitionType, HealthStatus, InjurySeverity } from '../types';
...
export const BackgroundMatchProcessor = {
    processLeagueEvent: (currentDate, userTeamId, fixtures, clubs, playersMap, lineups, seasonNumber, coaches, 
    ...
    sessionSeed = 0) => {
        ...
        const todayFixtures = fixtures.filter(f => f.date.toDateString() === dateStr &&
            f.status === MatchStatus.SCHEDULED &&
            !CL_COMPETITION_IDS.has(f.leagueId));
        // DEBUG
        ...
        if (todayFixtures.length === 0) {
            ...
            if (currentDate.getDate() === 1) {
                scoutedPlayers = AiScoutingService.updateTransferInterests(transferResolvedUpdate.updatedClubs, transferResolvedUpdate.updatedPlayers, currentDate, userTeamId, sessionSeed);
                scoutedPlayers = AiContractService.processMonthlyPlayerReview(transferResolvedUpdate.updatedClubs, scoutedPlayers, currentDate, userTeamId).updatedPlayers;
            }
            return {
        ...
        todayFixtures.forEach(fixture => {
            ...
            // NOWY SILNIK V2.0:
            const result = LeagueBackgroundMatchEngineV2.simulate(fixture, home, away, hPlayers, aPlayers, hLineup, aLineup, hCoach, aCoach, assignedRef, weather, seed);
            ...
            const leagueStandings = standingsMap[fixture.leagueId] || [];
            ...
            MatchHistoryService.logMatch({
                ...
                cards: (() => {
                    ...
                    result.cards.forEach(c => {
                        ...
                        if (!playerMatchCards[pId])
                            playerMatchCards[pId] = [];
            ...
            if (fixture.leagueId === 'L_PL_1')
                roundResults.league1Results.push(matchResult);
            else if (fixture.leagueId === 'L_PL_2')
                roundResults.league2Results.push(matchResult);
            else if (fixture.leagueId === 'L_PL_3')
                roundResults.league3Results.push(matchResult);
            currentClubs = currentClubs.map(c => {
                if (c.id === home.id || c.id === away.id) {
                    ...
                    const resultChar = pts === 3 ? 'W' : (pts === 1 ? 'R' : 'P');
                    const newForm = [...(c.stats.form || []), resultChar].slice(-5);
                ...
                if (clubId === home.id || clubId === away.id) {
                    currentPlayers[clubId] = currentPlayers[clubId].map(p => {
                        ...
                        if (result.ratings && result.ratings[p.id]) {
                            if (!updatedP.stats.ratingHistory)
                                updatedP.stats.ratingHistory = [];
        ...
        if (currentDate.getDate() === 1) {
            currentPlayers = AiScoutingService.updateTransferInterests(currentClubs, currentPlayers, currentDate, userTeamId, sessionSeed);
            currentPlayers = AiContractService.processMonthlyPlayerReview(currentClubs, currentPlayers, currentDate, userTeamId).updatedPlayers;
        }
        return {
