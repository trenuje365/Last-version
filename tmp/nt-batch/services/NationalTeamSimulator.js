...
exports.NationalTeamSimulator = void 0;
const types_1 = require("../types");
const LineupService_1 = require("./LineupService");
const NationalTeamEnvironmentService_1 = require("./NationalTeamEnvironmentService");
const NationalTeamLineupService_1 = require("./NationalTeamLineupService");
...
const nameOf = (p) => p ? `${p.firstName} ${p.lastName}` : 'Unknown';
const fatMul = (f) => 0.52 + 0.48 * Math.pow(clamp(f, 0, 100) / 100, 1.25);
const clonePlayer = (p) => ({
    ...p,
    stats: { ...p.stats },
...
const locMap = (m) => {
    const out = {};
    for (const [clubId, squad] of Object.entries(m))
        squad.forEach((p, index) => { out[p.id] = { clubId, index }; });
    return out;
};
const fallbackCoach = (teamId) => ({
    id: `NT_COACH_${teamId}`,
    firstName: 'National',
    lastName: 'Coach',
    age: 50,
...
const activePlayers = (lt) => lt.activeXI.map(id => lt.squad.find(p => p.id === id) ?? null).filter(Boolean);
const slotIndex = (lt, playerId) => lt.activeXI.findIndex(id => id === playerId);
const slotRole = (lt, playerId) => {
    const idx = slotIndex(lt, playerId);
    return idx >= 0 ? (tactics_db_1.TacticRepository.getById(lt.tacticId).slots[idx]?.role ?? types_1.PlayerPosition.MID) : types_1.PlayerPosition.MID;
};
const weighted = (players, rng, score) => {
...
const metrics = (lt) => {
    ...
    if (!act.length)
        return { att: 20, build: 20, create: 20, def: 20, press: 20, gk: 20, ment: 20, aggr: 20, avgFat: 0, active: 0 };
    let att = 0;
    let build = 0;
    let create = 0;
    let def = 0;
    ...
    act.forEach(p => {
        ...
        const ra = role === types_1.PlayerPosition.FWD ? 1.2 : role === types_1.PlayerPosition.MID ? 1.0 : role === types_1.PlayerPosition.DEF ? 0.72 : 0.18;
        const rd = role === types_1.PlayerPosition.DEF ? 1.2 : role === types_1.PlayerPosition.MID ? 0.96 : role === types_1.PlayerPosition.FWD ? 0.62 : 0.4;
        const rm = role === types_1.PlayerPosition.MID ? 1.18 : role === types_1.PlayerPosition.DEF ? 0.82 : role === types_1.PlayerPosition.FWD ? 0.88 : 0.25;
        ...
        if (role === types_1.PlayerPosition.GK || p.position === types_1.PlayerPosition.GK)
            gk += (p.attributes.goalkeeping * 0.62 + p.attributes.positioning * 0.18 + p.attributes.mentality * 0.1 + p.attributes.passing * 0.1) * m;
...
const eventPush = (timeline, minute, teamSide, type, text, primaryPlayerId, secondaryPlayerId) => {
    timeline.push({ minute, teamSide, type, text, primaryPlayerId, secondaryPlayerId });
};
const goalEntry = (scorer, teamId, minute, isPenalty, assistant) => ({
    playerId: scorer.id,
...
const pickCreator = (lt, rng) => weighted(activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK), rng, p => {
    const role = slotRole(lt, p.id);
    const rb = role === types_1.PlayerPosition.MID ? 1.18 : role === types_1.PlayerPosition.FWD ? 1.08 : 0.9;
...
const pickShooter = (lt, rng) => weighted(activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK), rng, p => {
    const role = slotRole(lt, p.id);
    const rb = role === types_1.PlayerPosition.FWD ? 1.22 : role === types_1.PlayerPosition.MID ? 0.95 : 0.7;
...
const pickDefender = (lt, rng) => weighted(activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK), rng, p => (p.attributes.defending * 1.45 + p.attributes.positioning * 1.2 + p.attributes.strength * 0.8 + p.attributes.pace * 0.72 + p.attributes.aggression * 0.68 + p.attributes.heading * 0.55 + p.attributes.mentality * 0.55) * fatMul(lt.fatigue[p.id] ?? 100));
const pickKeeper = (lt) => lt.squad.find(p => p.id === lt.activeXI[0]) ?? null;
const pickPenalty = (lt, rng) => {
    const act = activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK);
...
const removeFromPitch = (lt, playerId) => { lt.activeXI = lt.activeXI.map(id => id === playerId ? null : id); };
const benchReplacement = (lt, requiredRole, minute, losing) => {
    const bench = lt.bench.map(id => lt.squad.find(p => p.id === id) ?? null).filter(Boolean);
    if (!bench.length)
        return null;
    const sorted = [...bench].sort((a, b) => {
        const sa = LineupService_1.LineupService.calculateFitScore(a, requiredRole) + (lt.fatigue[a.id] ?? a.condition ?? 100) * 0.75 + (losing && a.position === types_1.PlayerPosition.FWD ? 10 : 0) + (losing && a.position === types_1.PlayerPosition.MID ? 6 : 0);
        const sb = LineupService_1.LineupService.calculateFitScore(b, requiredRole) + (lt.fatigue[b.id] ?? b.condition ?? 100) * 0.75 + (losing && b.position === types_1.PlayerPosition.FWD ? 10 : 0) + (losing && b.position === types_1.PlayerPosition.MID ? 6 : 0);
        return sb - sa;
    });
    return sorted.find(p => p.position === requiredRole) ?? (minute >= 72 && losing ? sorted.find(p => p.position !== types_1.PlayerPosition.GK) ?? sorted[0] : sorted[0]);
};
const doSub = (lt, slotIdx, incoming, minute, subs, timeline) => {
    ...
    eventPush(timeline, minute, lt.side, types_1.MatchEventType.SUBSTITUTION, `${lt.team.name}: ${nameOf(incoming)} for ${nameOf(outgoing)}`, incoming.id, outgoing?.id);
};
const maybeSub = (lt, minute, homeScore, awayScore, subs, timeline) => {
    if (lt.subs >= 5 || !lt.bench.length || minute < 46)
        return;
    const act = activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK);
    ...
    if (idx < 0)
        return;
    const req = tactics_db_1.TacticRepository.getById(lt.tacticId).slots[idx]?.role ?? target.position;
    const incoming = benchReplacement(lt, req, minute, losing);
    if (!incoming)
        return;
    const cur = LineupService_1.LineupService.calculateFitScore(target, req) * fatMul(lt.fatigue[target.id] ?? 100);
    const inc = LineupService_1.LineupService.calculateFitScore(incoming, req) * fatMul(lt.fatigue[incoming.id] ?? incoming.condition ?? 100);
    if (!injured && inc < cur * 0.84 && !losing)
        return;
    doSub(lt, idx, incoming, minute, subs, timeline);
};
const fatigueTick = (lt, minute, weatherInt, trailing) => {
    lt.activeXI.forEach((id, idx) => {
        ...
        if (!p)
            return;
        const role = tactics_db_1.TacticRepository.getById(lt.tacticId).slots[idx]?.role ?? p.position;
        let drain = 0.18;
        if (role === types_1.PlayerPosition.DEF)
            drain *= 1.18;
        if (role === types_1.PlayerPosition.MID)
            drain *= 1.25;
        if (role === types_1.PlayerPosition.FWD)
            drain *= 1.14;
        if (role === types_1.PlayerPosition.GK)
            drain *= 0.22;
...
const maybeInjury = (lt, minute, weatherInt, homeScore, awayScore, rng, injuries, subs, timeline) => {
    const act = activePlayers(lt).filter(p => p.position !== types_1.PlayerPosition.GK);
    if (!act.length)
        return;
    ...
    injuries.push({ playerId: injured.id, playerName: nameOf(injured), minute, teamId: lt.team.id, severity: severe ? types_1.InjurySeverity.SEVERE : types_1.InjurySeverity.LIGHT, days, type });
    eventPush(timeline, minute, lt.side, severe ? types_1.MatchEventType.INJURY_SEVERE : types_1.MatchEventType.INJURY_LIGHT, `${nameOf(injured)} injured for ${lt.team.name}`, injured.id);
    if (!severe && f > 46 && minute < 82)
        return;
    const idx = slotIndex(lt, injured.id);
    const req = idx >= 0 ? (tactics_db_1.TacticRepository.getById(lt.tacticId).slots[idx]?.role ?? injured.position) : injured.position;
    const losing = lt.side === 'HOME' ? homeScore < awayScore : awayScore < homeScore;
    const incoming = lt.subs < 5 ? benchReplacement(lt, req, minute, losing) : null;
    if (incoming && idx >= 0)
        doSub(lt, idx, incoming, minute, subs, timeline);
    else
        removeFromPitch(lt, injured.id);
};
const maybeCardOrPenalty = (att, def, minute, weatherInt, rng, goals, cards, timeline, homeRef, awayRef) => {
    ...
    const recklessness = clamp((defender.attributes.aggression / 100) * 0.36 +
        ((100 - defender.attributes.mentality) / 100) * 0.24 +
        ((100 - defender.attributes.defending) / 100) * 0.08 +
        fatigueRisk +
        weatherInt * 0.1 +
        (lateChasing ? 0.06 : 0), 0.08, 0.92);
    const foulChance = clamp(0.12 + Math.max(0, duelEdge) * 0.28 + recklessness * 0.18, 0.1, 0.3);
    if (rng.next() >= foulChance)
        return;
    const foulSeverity = clamp(0.22 + Math.max(0, duelEdge) * 0.5 + recklessness * 0.44 + (alreadyBooked ? 0.04 : 0), 0.14, 0.98);
    const cardChance = clamp(0.16 + Math.max(0, foulSeverity - 0.18) * 0.56, 0.12, 0.52);
    ...
    if (directRed) {
        ...
        eventPush(timeline, minute, def.side, types_1.MatchEventType.RED_CARD, `${nameOf(defender)} sent off for ${def.team.name}`, defender.id);
        ...
        if (alreadyBooked) {
            ...
            eventPush(timeline, minute, def.side, types_1.MatchEventType.YELLOW_CARD, `${nameOf(defender)} booked for ${def.team.name}`, defender.id);
        }
    }
    const penChance = clamp(0.009 + Math.max(0, creator.attributes.pace + creator.attributes.dribbling + creator.attributes.attacking - defender.attributes.positioning - defender.attributes.pace - defender.attributes.defending) / 3000 + Math.max(0, foulSeverity - 0.5) * 0.014 + weatherInt * 0.003, 0.006, 0.032);
    if (rng.next() >= penChance)
        return;
    ...
    eventPush(timeline, minute, att.side, types_1.MatchEventType.PENALTY_AWARDED, `Penalty for ${att.team.name}`, taker.id, defender.id);
    if (rng.next() < scoreChance) {
        goals.push(goalEntry(taker, att.team.id, minute, true));
        eventPush(timeline, minute, att.side, types_1.MatchEventType.PENALTY_SCORED, `${nameOf(taker)} scores the penalty for ${att.team.name}`, taker.id);
        if (att.side === 'HOME')
            homeRef.value += 1;
        else
            awayRef.value += 1;
        ...
        eventPush(timeline, minute, att.side, types_1.MatchEventType.PENALTY_MISSED, `${nameOf(taker)} misses the penalty for ${att.team.name}`, taker.id);
    }
};
const maybeGoal = (att, def, minute, weatherInt, rng, goals, timeline, attM, defM, scoreRef) => {
    const creator = pickCreator(att, rng);
    const shooter = pickShooter(att, rng);
    const defender = pickDefender(def, rng);
    const keeper = pickKeeper(def);
    if (!creator || !shooter || !defender || !keeper)
        return;
    ...
    if (rng.next() < goalChance) {
        ...
        eventPush(timeline, minute, att.side, types_1.MatchEventType.GOAL, `${nameOf(shooter)} scores for ${att.team.name}`, shooter.id, assistant?.id);
        scoreRef.value += 1;
    }
};
const updatePlayers = (updated, locs, date, home, away, goals, cards, injuries) => {
    const goalBy = {};
    const assistBy = {};
    const yellowBy = {};
    const redBy = {};
    const injBy = {};
    goals.forEach(g => { if (g.playerId)
        goalBy[g.playerId] = (goalBy[g.playerId] ?? 0) + 1; if (g.assistantId)
        assistBy[g.assistantId] = (assistBy[g.assistantId] ?? 0) + 1; });
    cards.forEach(c => { if (!c.playerId)
        return; if (c.type === 'YELLOW')
        ...
        redBy[c.playerId] = (redBy[c.playerId] ?? 0) + 1; });
    injuries.forEach(i => { if (i.playerId)
        injBy[i.playerId] = i; });
    [home, away].forEach(lt => {
        const ids = new Set([...Object.keys(lt.minutes), ...lt.activeXI.filter(Boolean)]);
        ids.forEach(id => {
            const loc = locs[id];
            if (!loc || !updated[loc.clubId]?.[loc.index])
                return;
            const next = clonePlayer(updated[loc.clubId][loc.index]);
            const mins = lt.minutes[id] ?? 0;
            if (mins > 0) {
                next.stats.matchesPlayed += 1;
                next.stats.minutesPlayed += mins;
            }
            ...
            if (lt.fatigue[id] !== undefined)
                next.condition = clamp(lt.fatigue[id], 0, 100);
            if (lt.debt[id] !== undefined) {
                next.fatigueDebt = clamp((next.fatigueDebt ?? 0) + lt.debt[id], 0, 100);
                next.condition = Math.min(next.condition, 100 - next.fatigueDebt);
            }
            if (injBy[id]) {
                const injury = injBy[id];
                next.health = { status: types_1.HealthStatus.INJURED, injury: { type: injury.type, daysRemaining: injury.days, severity: injury.severity, injuryDate: date.toISOString(), totalDays: injury.days, conditionAtInjury: next.condition } };
            }
            updated[loc.clubId][loc.index] = next;
        });
    });
};
const fallback = (match, comp, date, seed) => {
...
const singleMatch = (match, md, date, seed, season, updated, locs, byName, coaches) => {
    const homeTeam = byName.get(match.home) ?? null;
    const awayTeam = byName.get(match.away) ?? null;
    if (!homeTeam || !awayTeam)
        return { result: fallback(match, md.competitionLabel, date, seed), history: null };
    const squadOf = (team) => team.squadPlayerIds.map(id => { const loc = locs[id]; return loc ? updated[loc.clubId]?.[loc.index] ?? null : null; }).filter(Boolean);
    ...
    const hs = NationalTeamLineupService_1.NationalTeamLineupService.buildMatchSelection(homeTeam, homeSquad, homeCoach);
    const as = NationalTeamLineupService_1.NationalTeamLineupService.buildMatchSelection(awayTeam, awaySquad, awayCoach);
    const envSeed = `${seed}|${homeTeam.id}|${awayTeam.id}`;
    const weather = NationalTeamEnvironmentService_1.NationalTeamEnvironmentService.getWeather(date, homeTeam, awayTeam, md.competitionLabel, envSeed);
    const attendance = NationalTeamEnvironmentService_1.NationalTeamEnvironmentService.estimateAttendance(homeTeam, awayTeam, md.competitionLabel, weather, envSeed);
    ...
    const home = { side: 'HOME', team: homeTeam, coach: homeCoach, squad: homeSquad, activeXI: [...hs.lineup.startingXI], bench: [...hs.lineup.bench], tacticId: hs.lineup.tacticId, fatigue: Object.fromEntries(homeSquad.map(p => [p.id, p.condition ?? 100])), debt: {}, minutes: {}, yellows: {}, sentOff: new Set(), subs: 0, penaltyTakerId: hs.penaltyTakerId };
    const away = { side: 'AWAY', team: awayTeam, coach: awayCoach, squad: awaySquad, activeXI: [...as.lineup.startingXI], bench: [...as.lineup.bench], tacticId: as.lineup.tacticId, fatigue: Object.fromEntries(awaySquad.map(p => [p.id, p.condition ?? 100])), debt: {}, minutes: {}, yellows: {}, sentOff: new Set(), subs: 0, penaltyTakerId: as.penaltyTakerId };
    const goals = [];
    const cards = [];
    const injuries = [];
    const substitutions = [];
    ...
    for (let minute = 1; minute <= 90 + addedTime; minute++) {
        ...
        for (let i = 0; i < phases; i++) {
            ...
            const duelChance = clamp(0.1 + (attM.press + defM.aggr) / 12000 + (weather.weatherIntensity ?? 0) * 0.015, 0.1, 0.2);
            if (rng.next() < duelChance) {
                maybeCardOrPenalty(att, def, minute, weather.weatherIntensity ?? 0, rng, goals, cards, timeline, homeScore, awayScore);
                stop += 0.08;
            }
            else {
                ...
                if (last?.minute === minute && (last.type === types_1.MatchEventType.GOAL || last.type === types_1.MatchEventType.PENALTY_SCORED))
                    stop += 0.22;
            }
        }
...
exports.NationalTeamSimulator = {
    simulateMatchDay: (matchDay, dateSeed, currentDate, nationalTeams, players, coaches, seasonNumber) => {
        ...
        matchDay.matches.forEach((match, idx) => {
            ...
            results.push(result);
            if (history)
                matchHistoryEntries.push(history);
        });
