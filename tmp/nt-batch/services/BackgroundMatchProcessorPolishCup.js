import { MatchStatus, CompetitionType, HealthStatus, InjurySeverity, PlayerPosition } from '../types';
...
const simulateCupMatch = (home, away, hPlayers, aPlayers, hLineup, aLineup, seed, weatherEqualizer, isNeutralVenue) => {
    // ── SEEDED RNG ──────────────────────────────────────────────
    ...
    const referee = RefereeService.assignReferee(`${home.id}_${away.id}_${seed}`, 3); // 3 = ważność meczu
    ...
    const getTeamStrength = (players, xi) => {
        ...
        if (active.length === 0)
            return { att: 40, def: 40, gk: 40, overall: 40 };
    ...
    const redCardMultiplier = (redCount) => {
        if (redCount === 0)
            return 1.0;
        if (redCount === 1)
            return 0.72; // drużyna w 10 nadal strzela, tylko rzadziej
        if (redCount === 2)
            return 0.45;
        return 0.25;
    };
    ...
    const computeMinuteXG = () => {
        ...
        const REP_FLOOR = 1;
        const repRatio = Math.pow(Math.max(REP_FLOOR, home.reputation) / Math.max(REP_FLOOR, away.reputation), 0.3);
    ...
    const simulateMinutes = (fromMinute, toMinute) => {
        for (let minute = fromMinute; minute <= toMinute; minute++) {
            ...
            if (rng() < homeChance) {
                ...
                let goalMin = minute;
                while (usedGoalMinutes.has(goalMin) && goalMin <= toMinute + 5)
                    goalMin++;
    ...
    if (homeGoals === awayGoals) {
        ...
        const simulatePenaltySeries = (shooters, shooterXI, keeper) => {
...
export const BackgroundMatchProcessorPolishCup = {
    processCupEvent: (currentDate, userTeamId, fixtures, clubs, playersMap, lineups, careerSeed, seasonNumber) => {
        const dateStr = currentDate.toDateString();
        const todayCupFixtures = fixtures.filter(f => f.date.toDateString() === dateStr &&
        ...
        todayCupFixtures.forEach(fixture => {
            ...
            // ── SYMULACJA MECZU ────────────────────────────────────
            const result = simulateCupMatch(home, away, hPlayers, aPlayers, hLineup, aLineup, seed, weatherEqualizer, isNeutralVenue);
            ...
            currentClubs = currentClubs.map(c => {
                if (c.id === home.id)
                    return { ...c, isInPolishCup: isHomeWinner };
                if (c.id === away.id)
                    return { ...c, isInPolishCup: !isHomeWinner };
                return c;
            });
            // ── STATYSTYKI GRACZY ──────────────────────────────────
            const participatingIdsHome = [...hLineup.startingXI].filter(Boolean);
            const participatingIdsAway = [...aLineup.startingXI].filter(Boolean);
