...
import { MatchEventType, InjurySeverity, PlayerPosition } from '../types';
import { GoalAttributionService } from './GoalAttributionService';
export const LeagueBackgroundMatchEngineV2 = {
    simulate: (_fixture, _homeClub, _awayClub, homePlayers, awayPlayers, homeLineup, awayLineup, homeCoach, awayCoach, referee, weather, seed) => {
        // Generator liczb pseudolosowych na podstawie ziarna
        const seededRng = (offset) => {
        ...
        const getEffectivenessMult = (score) => {
            if (score <= 1)
                return 0.70; // Drastyczna przewaga przeciwnika
            if (score <= 3)
                return 0.85; // Odczuwalna trudność
            if (score === 4)
                return 1.00; // Balans
            if (score <= 6)
                return 1.15; // Przewaga taktyczna
        ...
        const homeFieldBonus = 1.0015 + (seededRng(2) * 0.0085);
        ...
        for (let minute = 1; minute <= 95; minute++) {
            ...
            const hActiveXI = homeLineup.startingXI.filter(id => id !== null);
            const aActiveXI = awayLineup.startingXI.filter(id => id !== null);
            ...
            if (!hHasRealGk)
                aGkPanic = 1.2 + (seededRng(minute + 55) * 0.6);
            if (!aHasRealGk)
                hGkPanic = 1.2 + (seededRng(minute + 55) * 0.6);
            ...
            if (seededRng(minute + 700) < penaltyProb) {
                ...
                if (isScored) {
                    if (side === 'H')
                        ...
                        awayScore++;
                    const kicker = GoalAttributionService.pickScorer(side === 'H' ? homePlayers : awayPlayers, (side === 'H' ? homeLineup : awayLineup).startingXI, false, () => seededRng(minute + 703));
                    if (!kicker)
                        break; // brak kandydatów (np. czerwona kartka wyczyściła skład)
        ...
        const calculateDebt = (lineup, players) => {
        ...
        const generateRating = (pId, isHome) => {
            ...
            pCards.forEach(c => {
                if (c.type === MatchEventType.RED_CARD)
                    score -= 3.0;
        ...
        const finalHomeXI = homeLineup.startingXI.filter(id => id !== null);
        const finalAwayXI = awayLineup.startingXI.filter(id => id !== null);
