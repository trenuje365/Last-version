import { PlayerPosition, MatchEventType, InjurySeverity } from '../types';
import { TacticRepository } from '../resources/tactics_db';
import { GoalAttributionService } from './GoalAttributionService';
export const LeagueBackgroundMatchEngine = {
    simulate: (fixture, homeClub, awayClub, homePlayers, awayPlayers, homeLineup, awayLineup, seed) => {
        ...
        const hStr = getLineStrength(homePlayers, homeLineup.startingXI);
        const aStr = getLineStrength(awayPlayers, awayLineup.startingXI);
        ...
        let homeRedCards = 0;
        let awayRedCards = 0;
        const preRollCards = (lineupIds, offset) => {
            let reds = 0;
            lineupIds.forEach((pId, idx) => {
                if (!pId)
                    return;
                if (seededRng(offset + idx + 1500) < 0.0033)
                    reds++; // Bezpośrednia czerwona
                else if (seededRng(offset + idx + 1000) < 0.087 && seededRng(offset + idx + 1200) < 0.05)
                    reds++; // Druga żółta
            });
            return reds;
        };
        ...
        const usedCardMinutes = new Map(); // minuta -> ilość kartek
        const getSafeMinute = (baseMin, type) => {
            let m = baseMin;
            if (type === 'GOAL') {
        ...
        const simulateSubs = (lineup, teamPlayers, sideOffset) => {
        ...
        const attributeGoals = (count, teamPlayers, lineup, isHome, subs) => {
        ...
        const simulateDetails = (lineupIds, players, offset) => {
            lineupIds.forEach((pId, idx) => {
                if (!pId)
                    return;
                ...
                let drain = 5.5 + seededRng(offset + idx) * 2.5 + stamEff + 2.5;
