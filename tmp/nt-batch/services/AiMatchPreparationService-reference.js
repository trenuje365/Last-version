    ...
    makeDecisions: (state, ctx, side, isPriority = false, isHalftime = false) => {
        ...
        if (gkInSlot === null && currentSubsCount < 5) {
            ...
            if (bestGkOnBench) {
                ...
                if (fieldPlayerIdx !== -1) {
                    ...
                    newLineup.bench.push(playerOutId);
                    ...
                    logs.push(`${isHalftime ? 'Przerwa' : state.minute + '\''} - AI: Bramkarz rezerwowy ${bestGkOnBench.lastName} wchodzi do gry.`);
                }
            }
        }
        ...
        if (state.minute > 20 && !newTacticId) {
            if (scoreDiff < -1 && state.minute > 45 && currentLineup.tacticId !== '3-4-3') {
                newTacticId = '3-4-3';
                logs.push(`${state.minute}' - AI: Drastyczna zmiana ustawienia na 3-4-3.`);
            }
            else if (scoreDiff > 0 && state.minute > 75 && currentLineup.tacticId !== '5-4-1') {
                newTacticId = '5-4-1';
                logs.push(`${state.minute}' - AI: Zagęszczenie obrony, przechodzimy na 5-4-1.`);
            }
        }
        return {
            newLineup,
