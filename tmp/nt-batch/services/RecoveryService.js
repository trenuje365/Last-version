import { HealthStatus, TrainingIntensity } from '../types';
export const RecoveryService = {
    /**
     * Wykonuje dobową regenerację dla wszystkich zawodników.
     * daysCount: pozwala na precyzyjne odliczanie czasu.
     */
    applyDailyRecovery: (playersMap, currentDate, intensity, daysCount = 1) => {
        ...
        for (const clubId in updatedMap) {
            updatedMap[clubId] = updatedMap[clubId].map(player => {
                ...
                let ageModifier = 1.0;
                if (player.age <= 24)
                    ageModifier = 0.8; // Młode organizmy (bonus)
                else if (player.age <= 29)
                    ageModifier = 0.6; // Szczyt formy
                else if (player.age <= 34)
                    ageModifier = 0.17; // Powolny spadek
                else
                    ageModifier = 0.7; // Weterani (wolna regeneracja)
                ...
                if (updated.freeAgentClubLockouts) {
                    const currentSimDate = new Date(currentDate).setHours(0, 0, 0, 0);
                    updated.freeAgentClubLockouts = Object.fromEntries(Object.entries(updated.freeAgentClubLockouts).filter(([, lockoutUntil]) => new Date(lockoutUntil).setHours(0, 0, 0, 0) > currentSimDate));
                }
                return updated;
            });
        }
