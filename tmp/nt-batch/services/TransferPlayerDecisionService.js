const roundMoney = (value) => Math.max(50_000, Math.round(value / 5_000) * 5_000);
...
const getAgeFinancialWeights = (age) => {
    if (age <= 23) {
...
export const TransferPlayerDecisionService = {
    buildNegotiationPlan: (player, currentClub, targetClub, currentSquad, targetSquad, currentDate) => {
        ...
        const isForeignMove = !!currentClub.country &&
            !!targetClub.country &&
        ...
        if (player.age <= 24 && reputationDelta >= 2)
            salaryMultiplier -= 0.08;
        if (player.age <= 24 && reputationDelta < 0)
            salaryMultiplier += 0.15;
        if (reputationDelta > 0)
            salaryMultiplier += 0.08;
        if (reputationDelta === 0 && isForeignMove)
            salaryMultiplier += 0.18;
        if (reputationDelta < 0)
            salaryMultiplier += 0.30;
        if (player.isOnTransferList)
            salaryMultiplier -= 0.08;
        if (isNotFirstTeamPlayer)
            salaryMultiplier -= 0.06;
    ...
    evaluateMove: (offer, player, currentClub, targetClub, currentSquad, targetSquad, currentDate) => {
        const negotiationPlan = TransferPlayerDecisionService.buildNegotiationPlan(player, currentClub, targetClub, currentSquad, targetSquad, currentDate);
        ...
        const financialFit = salaryFit * financialWeights.salary +
            bonusFit * financialWeights.bonus +
            yearsFit * (financialWeights.years + financialWeights.total);
        let salaryScore = 0;
        if (salaryFit >= 1.12)
            salaryScore = 18;
        else if (salaryFit >= 1.0)
            salaryScore = 12;
        else if (salaryFit >= 0.92)
            salaryScore = 5;
        else
            salaryScore = -10;
        let bonusScore = 0;
        if (bonusFit >= 1.15)
            bonusScore = 12;
        else if (bonusFit >= 1.0)
            bonusScore = 8;
        else if (bonusFit >= 0.85)
            bonusScore = 3;
        else
            bonusScore = -6;
        let yearsScore = 0;
        ...
        if (financialFit < requiredFinancialFit ||
