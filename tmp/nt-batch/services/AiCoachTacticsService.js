...
const getTopLineAvg = (players, pos, topN) => {
    const line = players
...
export const AiCoachTacticsService = {
    ...
    decidePreMatchInstructions: (ownClub, ownCoach, userClub, userPlayers, userTacticId, seed) => {
        ...
        if (experience < 40) {
            ...
            if (userDefAvg < 58)
                signalScore += 2;
            else if (userDefAvg < 65)
                signalScore += 1;
            // Mocny atak gracza → obrona
            if (userFwdAvg > 72)
                signalScore -= 2;
            else if (userFwdAvg > 65)
                signalScore -= 1;
            // Taktyka gracza
            if (userTacticDefBias > 65)
                signalScore += 1;
            if (userTacticDefBias < 40)
                signalScore -= 1;
            // Reputacja
            if (repDiff >= 3)
                signalScore += 2;
    ...
    decideInMatchInstructions: (aiScoreDiff, // aiScore - userScore (perspektywa AI)
    aiMomentum, // pozytywny = AI dominuje, ujemny = gracz dominuje
    minute, decisionMaking, experience, lastGoalBoostMinute, seed) => {
