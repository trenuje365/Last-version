import { HealthStatus, InjurySeverity } from '../types';
...
export const AiScoutingService = {
    ...
    generateReport: (_playerClub, playerPlayers, playerLineup, coachExperience) => {
        ...
        const starterPlayers = starters
            ...
            .filter((p) => !!p);
        const injuredOrSuspended = playerPlayers.filter(p => p.health.status === HealthStatus.INJURED ||
        ...
        const realPower = starterPlayers.reduce((sum, p) => sum + (p.attributes.attacking + p.attributes.passing + p.attributes.defending), 0);
        ...
        const perceivedFatigueLevel = perceivedCondition >= 82 ? 'FRESH' :
    ...
    updateTransferInterests: (clubs, playersMap, currentDate, userTeamId, sessionSeed) => {
        ...
        for (const club of clubs) {
            ...
            for (const need of needs) {
                const positionCandidates = AiScoutingService._findCandidatesForPosition(need.position, need.urgency, club, allPlayers, clubs, coachSeed, currentDate);
     ...
     */
    _findCandidatesForPosition: (position, urgency, club, allPlayers, allClubs, coachSeed, currentDate) => {
        const idealOvr = 30 + club.reputation * 4.5;
        ...
        const candidates = allPlayers.filter(p => {
            ...
            if (p.clubId === club.id)
                return false;
            // W zasięgu OVR
            if (p.overallRating < minOvr || p.overallRating > maxOvr)
                return false;
            ...
            const affordabilityMultiplier = AiScoutingService._getTransferListAffordabilityMultiplier(p, club, sellerClub, idealOvr);
            if (estimatedCost > maxAffordableValue * affordabilityMultiplier && p.clubId !== 'FREE_AGENTS')
                return false;
            // Nie kontuzjowany ciężko (klub nie interesuje się kontuzjowanymi na długo)
            if (p.health.status === HealthStatus.INJURED && (p.health.injury?.daysRemaining || 0) > 60)
                return false;
            return true;
        });
        ...
        return candidates.map(player => {
            ...
            if (player.isOnTransferList)
                score += 15; // klub wystawił na sprzedaż
            ...
            if (daysToExpiry < 180)
                score += 12; // kontrakt wygasa → łatwiej pozyskać
            else if (daysToExpiry < 365)
                score += 6;
            if (player.clubId === 'FREE_AGENTS')
                score += 20; // wolny agent = zero kosztu transferowego
            ...
            if (player.isOnTransferList && player.age > 31) {
                ...
                const stillProductive = (player.position === 'FWD' && gp >= 5 && player.stats.goals / gp >= 0.25) ||
     ...
     */
    _opportunisticScouting: (club, allPlayers, coachSeed) => {
        const idealOvr = 30 + club.reputation * 4.5;
        ...
        const pool = allPlayers.filter(p => p.clubId !== club.id &&
    ...
    _evaluatePlayerPerformance: (player) => {
        ...
        if (player.position === 'FWD') {
            ...
            if (goalsPerGame < 0.08)
                return 0.25; // bardzo słaby strzelec → prawie odpada z radaru
            if (goalsPerGame < 0.18)
                return 0.65; // poniżej oczekiwań
            if (goalsPerGame >= 0.40)
                return 1.35; // wybitny strzelec → duże zainteresowanie
            if (goalsPerGame >= 0.28)
                return 1.15; // powyżej oczekiwań
            return 1.0;
        }
        if (player.position === 'MID') {
            // Pomocnik oceniany po wkładzie ofensywnym (gole + asysty łącznie)
            if (contribPerGame < 0.08)
                return 0.30; // niekreatywny pomocnik
            if (contribPerGame < 0.18)
                return 0.70;
            if (contribPerGame >= 0.35)
                return 1.25; // regularnie asystuje/strzela
            if (contribPerGame >= 0.25)
                return 1.10;
            return 1.0;
        }
        ...
        if (dangerCardsPerGame > 0.5)
            return 0.55; // zbyt często karany → ryzyko transferowe
        if (dangerCardsPerGame > 0.3)
            return 0.80;
        return 1.0;
    },
    /**
     ...
     */
    _youngTalentScouting: (club, allPlayers, allClubs, coachSeed) => {
        const idealOvr = 30 + club.reputation * 4.5;
        const talents = allPlayers.filter(p => {
            ...
            if (playerClub.reputation > club.reputation - 2)
                return false;
            return true;
        });
        return talents.map(player => {
            ...
            score += AiScoutingService._seededRandom(coachSeed + AiScoutingService._hashString(player.id) + 1337) * 10;
     ...
     */
    _tier4GemScouting: (observingClub, allPlayers, tier4ClubIds, allClubs, coachSeed, currentDate) => {
        if (observingClub.reputation <= 3)
            return [];
        const observingIdealOvr = 30 + observingClub.reputation * 4.5;
        const gems = allPlayers.filter(p => {
            if (!tier4ClubIds.has(p.clubId || ''))
                return false;
            const playerClub = allClubs.find(c => c.id === p.clubId);
            if (!playerClub)
                return false;
            const clubIdealOvr = 30 + playerClub.reputation * 4.5;
            if (p.overallRating < clubIdealOvr + 12)
                return false;
            if (p.overallRating < observingIdealOvr - 25)
                return false;
            if (p.health.status === HealthStatus.INJURED && (p.health.injury?.daysRemaining || 0) > 60)
                return false;
            return true;
        ...
        for (const player of gems) {
            ...
            if (AiScoutingService._seededRandom(discoverySeed) > discoveryChance)
                continue;
    ...
    _getTransferListMarketOpportunityBonus: (player, buyingClub, sellerClub, buyerIdealOvr) => {
        if (!player.isOnTransferList || !sellerClub)
            return 0;
        ...
        if (repDelta >= -1 && repDelta <= 2)
            ...
            bonus += 5;
        if (sellerClub.reputation >= buyingClub.reputation)
            bonus += 4;
        if (qualityVsSeller >= 4)
            bonus += 12;
        else if (qualityVsSeller >= 1)
            bonus += 8;
        else if (player.overallRating >= buyerIdealOvr + 2)
            bonus += 4;
        if (player.age <= 29)
            bonus += 3;
        if (player.age >= 33)
