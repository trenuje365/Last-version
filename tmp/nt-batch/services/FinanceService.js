...
exports.FinanceService = exports.MATCHDAY_COST_PARAMS = exports.VIP_BOX_REVENUE_PARAMS = exports.MATCHDAY_ADDITIONAL_REVENUE_PARAMS = void 0;
// ============================================================
...
// ============================================================
exports.MATCHDAY_ADDITIONAL_REVENUE_PARAMS = {
    //                             tier: [  0,    1,    2,    3,    4 ]
...
// ============================================================
exports.VIP_BOX_REVENUE_PARAMS = {
    base: 150_000,
...
const eurMillionsToPln = (amount) => Math.round(amount * EUR_TO_PLN_NBP_2026 * 1_000_000);
const EUROPEAN_TIER_BASE_REVENUE_EUR_M = {
    1: 190,
    2: 90,
    3: 50,
    4: 8,
};
const EUROPEAN_COUNTRY_FINANCE_FACTOR = {
    ENG: 2.40,
    ESP: 1.70,
    GER: 1.80,
    ITA: 1.45,
...
const EUROPEAN_CLUB_REVENUE_OVERRIDE_PLN = {
...
const isEuropeanCommercialClub = (club) => EUROPEAN_COMMERCIAL_LEAGUES.has(club.leagueId);
const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
const getEuropeanCommercialIndex = (club) => {
...
exports.FinanceService = {
    ...
    calculateEuropeanInitialBudget: (tier, reputation, country, clubName, stadiumCapacity = 15_000) => {
        ...
        const estimatedRevenueEurM = baseRevenueEurM *
            countryFactor *
            reputationFactor *
            stadiumFactor *
            continentalPremium *
    ...
    calculateAvailableFunds: (totalBudget, squad) => {
        const expenses = exports.FinanceService.calculateTotalSalaries(squad);
    ...
    calculateNewgenSalary: (clubBudget, overall, age) => {
        const wagePool = exports.FinanceService.getWagePool(clubBudget);
        const avgSquadSalary = wagePool / 31;
        const youthDiscount = age <= 17 ? 0.38 :
            age <= 19 ? 0.46 :
                age <= 21 ? 0.58 :
                    0.72;
        ...
        const fairMarketSalary = exports.FinanceService.getFairMarketSalary(overall);
    ...
    calculateMatchdayExpenses: (club, isHome, attendance) => {
        if (isEuropeanCommercialClub(club)) {
            ...
            if (isHome) {
                ...
                const fillMultiplier = fillRate >= 0.95 ? 1.30 :
                    fillRate >= 0.85 ? 1.18 :
                        fillRate >= 0.70 ? 1.08 : 1.00;
                const rawCost = (180_000 +
                    club.stadiumCapacity * (5.5 + marketIndex * 1.8) +
                    att * (7.0 + marketIndex * 2.4) +
                    club.reputation * (16_000 + marketIndex * 8_000)) * fillMultiplier;
        ...
        const p = exports.MATCHDAY_COST_PARAMS;
        if (isHome) {
            const att = attendance ?? 0;
            const fillRate = club.stadiumCapacity > 0 ? att / club.stadiumCapacity : 0;
            ...
            const fillMultiplier = fillRate >= 0.95 ? 1.50 :
                fillRate >= 0.85 ? 1.30 :
                    fillRate >= 0.70 ? 1.10 : 1.00;
            const rawCost = (p.home.baseCost[tier] +
                att * p.home.perFanCost[tier] +
                club.reputation * p.home.repScale[tier]) * fillMultiplier;
            return Math.min(p.home.maxCap[tier], Math.max(p.home.minFloor[tier], Math.floor(rawCost)));
        }
        // Wyjazd — koszty transportu, zakwaterowania, diet zawodników i sztabu
        const rawCost = p.away.baseCost[tier] +
    ...
    calculateMarketValue: (player, reputation, tier) => {
        ...
        let baseValue = 0;
        if (ovr >= 80)
            baseValue = 15000000 + (ovr - 80) * 1300000;
        else if (ovr >= 75)
            baseValue = 8500000 + (ovr - 75) * 840000;
        else if (ovr >= 70)
            baseValue = 4200000 + (ovr - 70) * 860000;
        else
            baseValue = 50000 + (ovr / 70) * 4150000;
        let multiplier = 1.0;
        switch (tier) {
            case 1:
                multiplier = 1.15;
        ...
        if (reputation > 5)
            multiplier += (reputation - 5) * 0.03;
        else if (reputation < 4)
            multiplier -= (4 - reputation) * 0.02;
    ...
    evaluateReleaseRequest: (player, club, squad) => {
        ...
        if (woz < 30)
            return { status: 'APPROVED', woz, reason: "Zarząd akceptuje Pana decyzję. Koszty są akceptowalne, a zawodnik nie jest kluczowy dla wizerunku klubu." };
        if (woz < 60)
            return { status: 'WARNING', woz, reason: "Zarząd ma pewne wątpliwości co do opłacalności tego ruchu. Ostatecznie ufa Pana osądowi, ale oczekuje wyników." };
        if (woz < 85)
            return { status: 'SOFT_BLOCK', woz, reason: "Wniosek odrzucony. Obecnie nie możemy sobie pozwolić na taką stratę finansową. Proszę spróbować za 3 miesiące." };
    ...
    evaluateContractLogic: (player, newSalary, newBonus, newEndDate, currentDate, clubReputation) => {
        ...
        const expectedSalary = player.annualSalary > 0
            ? player.annualSalary
            : exports.FinanceService.getFairMarketSalary(player.overallRating);
        const expectedBonus = exports.FinanceService.calculatePlayerBonusDemand(player, expectedSalary, clubReputation);
        ...
        if (player.age >= 32) {
            wSal = 0.4;
            wBon = 0.5;
            wLen = 0.1; // Weterani cenią gotówkę "na stół"
        }
        else if (player.age <= 23) {
            wSal = 0.7;
            wBon = 0.1;
            wLen = 0.2; // Młodzi chcą wysokiej pensji i statusu
        }
        // 6. Ocena Długości
        const proposedYears = (newEnd - now) / (365 * 24 * 60 * 60 * 1000);
        const remainingYears = (currentEnd - now) / (365 * 24 * 60 * 60 * 1000);
        let lengthScore = 1.0;
        if (proposedYears < remainingYears)
            lengthScore = 0.5;
        if (player.age > 33 && proposedYears >= 2)
            lengthScore = 1.3; // Starsi kochają stabilizację
    ...
    evaluateFASigningBoardDecision: (player, proposedSalary, proposedBonus, squad, club) => {
        ...
        const fairSalary = exports.FinanceService.getFairMarketSalary(player.overallRating);
    ...
    evaluateRenewalBoardDecision: (player, proposedSalary, proposedBonus, squad, club) => {
        ...
        const currentWageBill = exports.FinanceService.calculateCurrentWageBill(squad);
    ...
    calculateTicketPriceForClub: (club) => {
        if (!isEuropeanCommercialClub(club)) {
            const tier = exports.FinanceService.getClubTier(club);
            return exports.FinanceService.calculateTicketPrice(tier, club.reputation);
        }
        const marketIndex = getEuropeanCommercialIndex(club);
    ...
    calculateMatchTicketRevenue: (attendance, tier, reputation) => {
        const maxPrice = exports.FinanceService.calculateTicketPrice(tier, reputation);
        const minPrice = 20;
    ...
    calculateMatchTicketRevenueForClub: (attendance, club) => {
        if (!isEuropeanCommercialClub(club)) {
            const tier = exports.FinanceService.getClubTier(club);
            return exports.FinanceService.calculateMatchTicketRevenue(attendance, tier, club.reputation);
        }
        const maxPrice = exports.FinanceService.calculateTicketPriceForClub(club);
    ...
    calculateSeasonTicketRevenue: (stadiumCapacity, reputation, tier) => {
        ...
        const singlePrice = exports.FinanceService.calculateTicketPrice(tier, reputation);
    ...
    calculateSeasonTicketPackageForClub: (club) => {
        if (!isEuropeanCommercialClub(club)) {
            const tier = exports.FinanceService.getClubTier(club);
            const revenue = exports.FinanceService.calculateSeasonTicketRevenue(club.stadiumCapacity, club.reputation, tier);
            const ticketsSold = Math.floor(club.stadiumCapacity * (0.10 + ((club.reputation / 10) * 0.20)));
            const ticketPrice = exports.FinanceService.calculateTicketPrice(tier, club.reputation);
        ...
        const singleMatchPrice = exports.FinanceService.calculateTicketPriceForClub(club);
    ...
    calculateMatchdayAdditionalRevenues: (attendance, tier, reputation) => {
        ...
        const p = exports.MATCHDAY_ADDITIONAL_REVENUE_PARAMS;
    ...
    calculateMatchdayAdditionalRevenuesForClub: (attendance, club) => {
        if (!isEuropeanCommercialClub(club)) {
            const tier = exports.FinanceService.getClubTier(club);
            return exports.FinanceService.calculateMatchdayAdditionalRevenues(attendance, tier, club.reputation);
        }
        const marketIndex = getEuropeanCommercialIndex(club);
    ...
    calculateVIPBoxRevenue: (stadiumCapacity, reputation) => {
        const p = exports.VIP_BOX_REVENUE_PARAMS;
    ...
    calculateVIPBoxRevenueForClub: (club) => {
        if (!isEuropeanCommercialClub(club)) {
            const tier = exports.FinanceService.getClubTier(club);
            if (tier !== 1 || club.stadiumCapacity <= 15_000)
                return 0;
            return exports.FinanceService.calculateVIPBoxRevenue(club.stadiumCapacity, club.reputation);
        }
        if (club.stadiumCapacity < 4_000)
            return 0;
        const marketIndex = getEuropeanCommercialIndex(club);
    ...
    calculateLeagueFinishBonus: (position, tier) => {
        ...
        if (bonuses[position])
            return Math.floor(bonuses[position]);
