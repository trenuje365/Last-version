import { TransferTiming } from '../types';
import { FinanceService } from './FinanceService';
const MIN_POSITION_DEPTH = {
    GK: 2,
    DEF: 5,
    MID: 4,
    FWD: 3
};
const roundToNearest50k = (value) => Math.round(Math.max(100_000, value) / 50_000) * 50_000;
const getTimingPriceMultiplier = (timing) => {
    switch (timing) {
...
export const TransferSellerLogicService = {
    ...
    getNegotiationStance: (player, sellerClub, buyerClub, sellerSquad, currentDate, timing = TransferTiming.IMMEDIATE) => {
        const baseAskingPrice = TransferSellerLogicService.estimateAskingPrice(player, sellerClub, sellerSquad, currentDate);
        ...
        const blocksShortDelaySale = timing === TransferTiming.IMMEDIATE || timing === TransferTiming.IN_SIX_MONTHS;
        const protectedRivalSale = sameLeague &&
        ...
        const protectedTopElevenSale = sameLeague &&
            !player.isOnTransferList &&
            !sellerNeedsCash &&
            isTopEleven &&
            reputationGap <= 0 &&
    ...
    estimateAskingPrice: (player, sellerClub, sellerSquad, currentDate) => {
        ...
        if (top11Ids.includes(player.id))
            multiplier += 0.14;
        ...
        if (samePosition.length < minimumDepth)
            multiplier += 0.18;
        if (!bestReplacement)
            multiplier += 0.12;
        if (bestReplacement && player.overallRating - bestReplacement.overallRating >= 6)
            multiplier += 0.10;
        ...
        if (top11Ids.includes(player.id))
            minimumMultiplier = Math.max(minimumMultiplier, 1.08);
        if (sortedSquad.slice(0, 3).some(p => p.id === player.id))
            minimumMultiplier = Math.max(minimumMultiplier, 1.15);
        if (player.isUntouchable || sortedSquad[0]?.id === player.id)
            minimumMultiplier = Math.max(minimumMultiplier, 1.25);
        const rawPrice = Math.max(100_000, baseValue * Math.max(multiplier, minimumMultiplier));
        return Math.round(rawPrice / 50_000) * 50_000;
    },
    evaluateSellerDecision: (offer, player, sellerClub, buyerClub, sellerSquad, currentDate, negotiationContext) => {
        const openingStance = TransferSellerLogicService.getNegotiationStance(player, sellerClub, buyerClub, sellerSquad, currentDate, offer.timing);
        ...
        const askingPrice = roundToNearest50k(negotiationContext?.currentAskingPrice || openingStance.askingPrice);
        ...
        const normalizedCounterPrice = Math.max(roundToNearest50k(askingPrice * 0.97), Math.min(roundToNearest50k(askingPrice * 1.03), counterPrice));
        return {
            verdict: 'COUNTER',
