import { NationalTeam, WeatherSnapshot } from '../types';
...
type ClimateSeasonKey = 'winter' | 'spring' | 'summer' | 'autumn';

const CLIMATE_BY_CONTINENT: Record<string, ClimateProfile> = {
  europe: {
    winter: [-6, 6],
    spring: [4, 18],
    summer: [14, 30],
...
const createSeeded = (seed: string) => {
...
const getSeasonKey = (month: number): ClimateSeasonKey => {
...
export const NationalTeamEnvironmentService = {
  getWeather: (
    date: Date,
  ...
  estimateAttendance: (
    ...
    const weatherPenalty =
      weather.weatherIntensity && weather.weatherIntensity >= 0.7
        ? 0.18
